...
<script type="text/javascript">
	...
	// hide the ID field
	$('form.call [name="idspec"]').hide();

	$('form.call [name="specialist"]').on('focus keyup',searchSpecs);

	$('form.call [name="specialist"]').blur(function(){
		$('form.call #specialists #found').slideUp();
	});

	function searchSpecs() {
		var search = $('form.call [name="specialist"]').val();
		if($.trim(search)) {
			$.ajax({
				type: "GET",
				...
				data: {'s':search},
				success: function(data) {
					$('form.call #specialists #found').html(data);
					$('form.call #specialists #found').slideDown();
					specButtons();
				},
				error: function(error) { // when there's a link to a page that doesn't exist
			...
			$('form.call #specialists #found').slideUp();
			$('form.call #specialists #found').html('');
		}
	}

	function specButtons() {
		$('form.call #specialists button').off('click vclick').on('click vclick', function(e) {
			e.preventDefault();
			$('form.call [name="idspec"]').val($(this).data('id'));
			$('form.call #specSearch').slideUp();
			$('form.call #assigned').html('');
			$('<span>Assigned to <strong>'+$(this).parent().siblings(':nth-of-type(2)').text()+'</strong></span><br>').hide().appendTo('#assigned').fadeIn();
			$('form.call #specialists #recommended').hide();
			$('<a href="#" class="delete">Remove</a><br><br>').hide().fadeIn().appendTo('#assigned').on('click vclick',function(e){
				e.preventDefault();
				$('form.call [name="idspec"]').val('')
				$('form.call #specSearch').slideDown();
				$('form.call #assigned').fadeOut();
				$('form.call #specialists #recommended').show();
			});
		});
	}
	...
	$('form[id^="add"] [name="specialist"]').blur(function(){
